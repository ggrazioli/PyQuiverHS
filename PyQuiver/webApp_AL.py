...
def calculate_kie():
    ...
    # create zip folder for outputs (.txt and plots)
    ...
    # process .txt files and generate plots
    kie_data = {}
    ...
    while start_temp <= final_temp: 
        ...
        if os.path.exists(output_file_path):
            ...
            for line in lines[5:]:      # skip 4-line header
    ...
    for isotopologue_name, data in kie_data.items():
        ...
        for i, kie_label in enumerate(kie_labels):
            plt.plot(data['temperature'], [kie_list[i] for kie_list in data['kies']], marker = 'o', label = kie_label)
