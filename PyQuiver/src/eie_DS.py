...
class EIE_Calculation(object):
    ...
    def __str__(self):
        ...
        if self.eie_flag == 0:
            ...
            string += "Isotopologue {1: ^10s} {0: ^12s} {2: ^16s} {3: ^16s} {4: ^14s} {5: ^14s} {6: ^14s} {7: ^14s} {8: ^14s} {9: ^14s} {10: ^14s} {11: ^14s} {12: ^14s}".format("", "Name", "EIE(TH)", "EIE(BM)", "Enthalpy", "Entropy", "H_ZPE", "H_VIB", "S_VIB", "S_ROT", "Approx. MMI", "EXC", "ZPE\n")
        else:
        ...
        if self.config.reference_isotopologue != "default" and self.config.reference_isotopologue != "none":
            string += "\n\nEIEs referenced to isotopologue {0}, whose absolute EIEs are:".format(self.config.reference_isotopologue)
...
class KIE(object):
    ...
    def calculate_kie_components(self):
        ...
        MMI_factor = np.prod(partition_factors_gs[:, 0]) / np.prod(partition_factors_ts[:, 0])
    ...
    def __str__(self):
        if self.value is not None:
            if self.eie_flag == 1:
                ...
                return "Isotopologue {1: ^10s} {0: ^12s} {2: ^16.8f} {3: ^14.8f} {4: ^14.8f} {5: ^14.8f} {6: ^14.8f} {7: ^14.8f} {8: ^14.8f} {9: ^14.8f} {10: ^14.8f} {11: ^14.8f} {12: ^14.8f}".format("", self.name, self.value[0], self.value[1], self.kie_components[0], self.kie_components[1], self.kie_components[2], self.kie_components[3], self.kie_components[4], self.kie_components[5], self.kie_components[6], self.kie_components[7], self.kie_components[8])
                # return "Isotopologue {1: >10s} {0: >33s} {2: ^12.8f}".format("", self.name, self.value)
        else:
