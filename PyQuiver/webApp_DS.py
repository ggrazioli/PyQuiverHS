...
def eie():
    if request.method == 'GET':
        ...
        column_names = ['Isotopologue', 'Temperature', 'EIE (TH)', 'EIE (BM)', 'Enthalpy', 'Entropy', 'H_ZPE', 'H_VIB', 'S_VIB', 'S_ROT', 'Approx. MMI', 'EXC', 'ZPE']
        df = pd.DataFrame(columns=column_names)
        ...
        while start_temp <= final_temp: 
            ...
            if os.path.exists(output_file_path):
                ...
                for line in lines[4:]:      # skip 5-line header
                    ...
                    eie_values = list(map(float, line_components[2:4]))     # this does NOT contain enthalpy and entropy
        ...
        for isotopologue_name, data in eie_data.items():
            ...
            for i, eie_label in enumerate(eie_labels):
                plt.plot(data['temperature'], [eie_list[i] for eie_list in data['eies']], marker = '.', label = eie_label)
        ...
        return send_file(os.path.join(original_cwd, SESSION_FOLDER, 'outputs.zip'), mimetype="application/zip", as_attachment=True, download_name=f"{SESSION_FOLDER}.zip")
